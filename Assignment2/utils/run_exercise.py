...
class ExerciseRunner:
    ...
    def __init__(self, topo_file, log_dir, pcap_dir,
                       switch1_json, switch2_json, switch3_json, switch4_json, bmv2_exe='simple_switch', quiet=False):
        ...
        self.switch1_json = switch1_json
        self.switch2_json = switch2_json
        self.switch3_json = switch3_json
        self.switch4_json = switch4_json
    ...
    def do_net_cli(self):
        ...
        if self.switch1_json:
            ...
            print('')
        if self.switch2_json:
            print('To inspect or change the switch configuration, connect to')
            print('its CLI from your host operating system using this command:')
            print('  simple_switch_CLI --thrift-port <switch thrift port>')
            print('')
        if self.switch3_json:
            print('To inspect or change the switch configuration, connect to')
            print('its CLI from your host operating system using this command:')
            print('  simple_switch_CLI --thrift-port <switch thrift port>')
            print('')
        if self.switch4_json:
            print('To inspect or change the switch configuration, connect to')
            print('its CLI from your host operating system using this command:')
            print('  simple_switch_CLI --thrift-port <switch thrift port>')
            print('')
        ...
        # @Lorenzo: do mininet-related stuff here
        #self.net.pingAll()
            
        # pingoutput = buf.getvalue()
        CLI(self.net)
        # sys.stdout.write(pingoutput)
        # sys.stdout.flush()
...
def get_args():
    ...
    parser.add_argument('-j1', '--switch1_json', type=str, required=False)
    parser.add_argument('-j2', '--switch2_json', type=str, required=False)
    parser.add_argument('-j3', '--switch3_json', type=str, required=False)
    parser.add_argument('-j4', '--switch4_json', type=str, required=False)
...
if __name__ == '__main__':
    ...
    exercise = ExerciseRunner(args.topo, args.log_dir, args.pcap_dir,
                              args.switch1_json, args.switch2_json, args.switch3_json, args.switch4_json, args.behavioral_exe, args.quiet)
